...
export default async function EditLlmModelPage({
  ...
  const [{ data: model }, { data: providers }] = await Promise.all([
    db
      ...
      .eq('id', id)
      .single(),
    db.from('llm_providers').select('id, name').order('id'),
  ])
  if (!model) notFound()
  ...
  return (
    <div className="p-8 max-w-lg">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">Edit LLM Model</h1>
      </div>
      <form action={updateLlmModel} className="bg-slate-800 rounded-xl p-6 space-y-5">
        ...
        <div>
          ...
          <input name="name" required defaultValue={model.name} className={inputClass} />
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1.5">Temperature Supported</label>
          <select
